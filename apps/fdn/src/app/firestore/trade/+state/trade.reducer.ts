...
export const tradeReducer = createReducer(
    initialState,
    on(TRADE_ACTIONS.TRADE_ADD_SUCCESS, (state, { trade }) => (
        {...state, trades: [...state.trades, trade] })
    ),
    on(TRADE_ACTIONS.TRADE_GET_SUCCESS, (state, { trades }) => (
        {...state, trades: [...trades] })
    ),
    on(TRADE_ACTIONS.TRADE_DEL_SUCCESS, (state, { trade }) => (
        {...state, trades: state.trades.filter(t => t.key !== trade.key) })
    ),
