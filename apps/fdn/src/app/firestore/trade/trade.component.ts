...
export class TradeComponent implements OnInit {
  ...
  constructor(
    private ts: TradeFacade,
    private dialog: MatDialog,
    private af: AuthenticationFacade
  ) {}
  ...
  performConsolidation(consolidate: TradeDocument[]): TradeDocument {
    ...
    if (consolidated.shares === 0) {
      _price = -consolidated.invested;
    } else {
      _price = this.round(consolidated.invested / consolidated.shares);
    }
    ...
    const trade: TradeDocument = {
      key: last.key,
      user: last.user ? last.user : this.uid,
      timestamp: last.timestamp,
      ticker: consolidated.ticker,
      shares: consolidated.shares,
      price: _price,
      commission: consolidated.commission
    };
